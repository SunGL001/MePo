...
class PromptVitNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        if args.get("meta_path", 0) != 0:  
                model_infos = torch.load(args["meta_path"])
                filtered_model_infos = {k: v for k, v in model_infos.items()
                ...
                if 'pos_embed' in model_dict:
                    ...
                    current_pos_embed = self.backbone.pos_embed.data # (1,222,768)
                    current_pos_embed[:, :197, :] = pretrained_pos_embed
                    self.backbone.pos_embed.data = current_pos_embed
        ...
        if args["get_original_backbone"]:
            ...
            if args.get("meta_path", 0) != 0:  
                ...
                filtered_model_infos = {k: v for k, v in model_infos.items()
                    if not k.startswith("head.")}
                self.original_backbone.load_state_dict(model_infos, strict=False)
                print("load original_backbone successfully")
...
class SPromptVitNet(nn.Module):
    def __init__(self, args:dict, pretrained:bool):
        ...
        if args.get("meta_path", 0) != 0:  
            ...
            if 'pos_embed' in model_dict:
                pretrained_pos_embed = filtered_model_infos["pos_embed"] # (1,197,768)
                current_pos_embed = self.image_encoder.pos_embed.data # (1,222,768)
                current_pos_embed[:, :197, :] = pretrained_pos_embed
                self.image_encoder.pos_embed.data = current_pos_embed
            self.image_encoder.load_state_dict(pretrained_dict, strict=False)
            print("load backbone successfully")
...
class SLCANet(BaseNet):
    ...
    def __init__(self, args, pretrained=True, fc_with_ln=False):
        ...
        if args.get("meta_path", 0) != 0:
            model_infos = torch.load(args["meta_path"])
            # import pprint
            # print(f"model_infos: {type(model_infos)}")
            # pprint.pprint(f"model_infos: {model_infos.keys()}")
            # pprint.pprint(f"self.backbone.state_dict().keys(): {self.backbone.state_dict().keys()}")
            # model_infos = {k.replace('vit.', ''): v for k, v in model_infos.items()}
            self.backbone.load_state_dict(model_infos, strict=False)
            print("load backbone successfully")
